    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                data = response.json()
                return {"source": "AbuseIPDB", "status": "success", "count": len(data.get("data", []))}
    except Exception as e:
        return {"source": "AbuseIPDB", "status": "error", "error": str(e)}
    
    return {"source": "AbuseIPDB", "status": "error", "error": "Request failed"}
...
async def fetch_otx():
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                data = response.json()
                return {"source": "OTX", "status": "success", "count": len(data.get("results", []))}
    except Exception as e:
